...
$excel->getProperties()->setCreator("Indah Jaya, PT.")
                       ...
                       ->setDescription("Report Detail Weekly Jahit")
...
if(!empty($dt) && !empty($hg))
{
    ...
    $sql = "select a.transaksi_hasil_jahit_date,
                   ...
                   a.jenis_id,
                   c.ukuran_detail_id,
                   a.transaksi_hasil_jahit_jam_kerja,
                   c.jumlah,
                   b.transaksi_hasil_jahit_detail_jumlah,
                   b.transaksi_hasil_jahit_detail_premi
    ...
    while($row = mysql_fetch_assoc($res))
    {
        $data[$row['transaksi_hasil_jahit_date']]
             ...
             [$row['transaksi_hasil_jahit_jam_kerja']] = array("jumlah"=>$row['jumlah'],
                                                               "premi"=>$row['transaksi_hasil_jahit_detail_premi'],
                ...
                foreach($karyawan as $kKaryawan => $vKaryawan)
                {
                    ...
                    foreach($ukuranDet as $kUkuran => $vUkuran)
                    {
                        ...
                        if($iteration == $countIterasion-1)
                        {
                            ...
                            $rw+=1;
                            //=================== start total pcs =======================//
                            $excel->getActiveSheet()->setCellValueByColumnAndRow($cl,$rw,"TOTAL (PCS)")
