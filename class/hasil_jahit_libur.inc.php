...
class hasil_jahit_libur
{
    ...
    function __construct() 
    {
        ...
        $this->tblName = "jht_transaksi_hasil_jahit_libur";
        $this->tblNameDet = "jht_transaksi_hasil_jahit_libur_detail";
        $this->tblNameDetUkuran = "jht_transaksi_hasil_jahit_libur_detail_ukuran";
        ...
        $this->sqlSearchDet = "select jthjd.transaksi_hasil_jahit_detail_id,
                               ...
                               from ".$this->tblName." jthj,
                                    ".$this->tblNameDet." jthjd
                                            ...
                                            --   left join ".$this->tblNameDetUkuran." x on x.transaksi_hasil_jahit_detail_id = jthjd.transaksi_hasil_jahit_detail_id
        ...
        $this->sqlSearchUkuranDet = "select jthjdu.transaksi_hasil_jahit_detail_ukuran_id,
                                            ...
                                            jthjdu.jam_kerja,
                                            jthjdu.modifiy_date
                                    from ".$this->tblNameDetUkuran." jthjdu
                                    where 1=1";
    }
...
//                $this->hitung_premi($this->master['HasilJahitDetIdUkuran']);
    ...
    function jum_data_det_ukuran($param1,$param2,$param3)
    {
        $sql = $this->sqlSearchUkuranDet;
        $sql .= " and jthjdu.transaksi_hasil_jahit_detail_id='".$param1."' and jthjdu.ukuran_detail_id = '".$param2."' and jthjdu.jam_kerja='".$param3."'";
        $res = mysql_query($sql);
        $jum = mysql_num_rows($res);
    ...
    function hitung_premi($param)
    {
        ...
        $sql = "select a.transaksi_hasil_jahit_detail_id, a.ukuran_detail_id,aa1.jenis_id,b1.ukuran_desc, b.ukuran_id,sum(a.jumlah) jml
                from jht_transaksi_hasil_jahit_libur_detail_ukuran a,
                          jht_master_ukuran_detail b left join jht_master_ukuran b1
                                on b1.ukuran_id = b.ukuran_id,
                          jht_transaksi_hasil_jahit_libur_detail aa left join jht_transaksi_hasil_jahit_libur aa1
                                on aa.transaksi_hasil_jahit_id = aa1.transaksi_hasil_jahit_id
                where a.transaksi_hasil_jahit_detail_id = '$param'
                      and a.ukuran_detail_id = b.ukuran_detail_id
                      and a.transaksi_hasil_jahit_detail_id = aa.transaksi_hasil_jahit_detail_id
                group by b.ukuran_id";
        
        $res = mysql_query($sql);
        while($row = mysql_fetch_assoc($res))
        {
            $premi = $this->get_premi($row['jenis_id'], $row['ukuran_id'], $row['jml']);
            
            //$totalPremi = $row['jml'] * $premi;
             ...
             * Update detail
             */
            $sqlUpd = "update jht_transaksi_hasil_jahit_libur_detail_ukuran a 
                       set a.premi = '$premi'
                       where a.transaksi_hasil_jahit_detail_id = '".$row['transaksi_hasil_jahit_detail_id']."'
                             and a.ukuran_detail_id = '".$row['ukuran_detail_id']."'";
            $resUpd = mysql_query($sqlUpd);
        }
    }
    
    function get_premi($jenis_id, $size_id, $sum)
    {
        $ret = 0;
        
        if(!empty($jenis_id) && !empty($size_id) && !empty($sum))
        {
            $sql = "select b.premi
                    from jht_index_jahit_libur a,
                         jht_index_jahit_libur_detail b
                    where a.index_id = b.index_id
                          and a.jenis_id = '$jenis_id'
                          and b.size_id = '$size_id'
                          and (
                                ('$sum'>=b.target_from and '$sum'<=b.target_until)
                                 or
                                ('$sum'>=b.target_max and b.target_max<>0)
                              )";
            $res = mysql_query($sql);
	...
	function hitung_premi_calc()
    {
        ...
        $sqlPremi = "select b.index_target,
                            b.target_min,
                            b.target_from,
                            b.target_until,
                            b.target_max,
